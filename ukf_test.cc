...
Ukf GetDummyEkfObject() {
  ...
  ukf.lambda = 3 - ukf.n_aug;
  //set example state
  ukf.x = VectorXd(ukf.n_x);
  ukf.x << 5.7441,
    1.3800,
    2.2049,
    0.5015,
  ...
  ukf.P = Matrix(ukf.n_x, ukf.n_x);
  ukf.P << 0.0043, -0.0013, 0.0030, -0.0022, -0.0020,
...
//
//  ukf.P = MatrixXd::Zero(5, 5);
//  ukf.PredictMeanAndCovariance(Xsig_pred);
//
//  REQUIRE(ukf.x.isApprox(output_x, 1e-5));
//  REQUIRE(ukf.P.isApprox(output_P, 1e-3));
//}
//
...
//  ukf.PredictRadarMeasurement(Xsig_pred, &z_out, &S_out, &ukf.Zsig_pred);
//
...
//  ukf.PredictLidarMeasurement(Xsig_pred, &z_out, &S_out, &ukf.Zsig_pred);
//
//  REQUIRE(z_out.isApprox(output_z, 1e-6));
//  REQUIRE(S_out.isApprox(output_S, 1e-3));
//
...
//
//  cout << "ukf.x: " << ukf.x << endl;
//  cout << "ukf.P: " << ukf.P << endl;
//  REQUIRE(output_x.isApprox(ukf.x, 1e-2));
//  REQUIRE(output_P.isApprox(ukf.P, 1e-2));
//
//}
